...
// 실제 이미지 파일이 있는 경로에 맞게 수정해주세요.
import cardFrontImage from './card_front.png'; // 예시: 컴포넌트와 같은 폴더에 이미지가 있는 경우
import cardBackImage from './card_back.png';   // 예시: 컴포넌트와 같은 폴더에 이미지가 있는 경우
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-100">
        <p className="text-xl text-gray-700">괘 데이터를 불러오는 중...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen bg-gray-100 p-4">
      <div className="flip-card w-[600px] h-[337px] border border-gray-200 perspective-1000 cursor-pointer" onClick={handleCardClick}>
        <div className={`flip-card-inner relative w-full h-full text-center transition-transform duration-600 transform-style-preserve-3d ${isFlipped ? 'is-flipped' : ''}`}>          <div className="flip-card-front absolute w-full h-full backface-hidden rounded-xl flex justify-center items-center text-2xl text-white font-bold relative">
            {/* 앞면: 문제/질문 - 괘상, 이름, 핵심관점 */}
            <Image
          ...
          </div>          <div className="flip-card-back absolute w-full h-full backface-hidden rounded-xl flex justify-center items-center text-2xl text-white font-bold relative">
            {/* 뒷면: 답/설명 - 요약, 키워드 */}
            <Image
        ...
        </div>      </div>      <div className="mt-6 flex space-x-4">
        <button
