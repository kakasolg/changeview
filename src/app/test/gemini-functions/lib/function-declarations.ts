...
//
// **A-2 세트 워크플로우 (3단계 연속 호출 필수)**:
// 1. analyze_user_situation: 사용자 입력 분석 → 감정/상황/키워드 추출
// 2. calculate_hexagram_compatibility: 1단계 결과로 64괘 적합도 점수 계산
// 3. select_final_hexagram: 2단계 점수로 최종 괘 선택 및 근거 제시
// 학분: 사용자 상담 완성을 위해서는 세 함수가 모두 순차적으로 호출되어야 합니다.
//
...
export const analyzeUserSituationDeclaration = {
  ...
  description: "사용자의 입력 텍스트를 분석하여 감정, 상황, 키워드를 추출합니다. **이 결과는 반드시 calculate_hexagram_compatibility 함수에 전달되어야 합니다.**",
  parameters: {
    type: Type.OBJECT,
    properties: {
      userInput: {
...
export const calculateHexagramCompatibilityDeclaration = {
  ...
  description: "사용자 상황 분석 결과를 바탕으로 64괘 각각의 적합도 점수를 계산합니다. **이 점수들은 반드시 select_final_hexagram 함수에 전달되어 최종 괘 선택을 완료해야 합니다.**",
  parameters: {
    type: Type.OBJECT,
    properties: {
      emotions: {
...
export const selectFinalHexagramDeclaration = {
  ...
  description: "적합도 점수와 분석 결과를 바탕으로 최종 괘을 선택하고 근거를 제시합니다. **이 함수로 사용자 상담 프로세스가 완성됩니다.**",
  parameters: {
    type: Type.OBJECT,
    properties: {
      compatibilityScores: {
